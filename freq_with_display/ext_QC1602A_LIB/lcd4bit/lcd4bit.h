...
#define LCD_DATA_PIN            PINA    // the data pin
#define LCD_DATA_START_INDEX    0       // the offset of the data pins in the 
                                        // port
//-----------------------------------------------------------------------------
// CONTROL PORT
//-----------------------------------------------------------------------------
...
void waitForIdleLCD() {
    ...
    do {
        ...
        status = (LCD_DATA_PIN >> LCD_DATA_START_INDEX);
        status = status << 4;

        processDataLCD();

        temp = (LCD_DATA_PIN >> LCD_DATA_START_INDEX);
        temp &= 0x0F;
        processDataLCD();
...
void initLCD() {
    ...
    LCD_DATA_PORT &= (~(0x0F << LCD_DATA_START_INDEX));
    // small delay
    _delay_us(5);
    // get first byte part by truncating half of the data shifting it right and pussing it to the data index
    // use either INIT_4BIT_1_LINE_5b7 or INIT_4BIT_2_LINE_5b7 as they have the same upper byte part
    LCD_DATA_PORT |= (INIT_4BIT_1_LINE_5b7 >> 4) << LCD_DATA_START_INDEX;
    // small delay to let the LCD set itself
    _delay_us(50);

    // set the lcd lines number
    if (LCD_LINES == 2) {
        commandLCD(INIT_4BIT_2_LINE_5b7);
    } else {
        commandLCD(INIT_4BIT_1_LINE_5b7);
    }
    _delay_us(50);
...
void sendLCD(unsigned char data, uint8_t isCommand) {
    waitForIdleLCD();
    // we will store the first byte part and second byte part here
    uint8_t fbp, sbp;
    uint8_t buffer;

    // first get the firt byte part by shifting the data right by 4
    fbp = data >> 4;
    // get the second part of the byte by & with mask 0b00001111
    sbp = (data & 0x0F);
    ...
    // delay
    SET_E();
    // make a 4 bit hole in LCD_DATA_PORT at the start of LCD_DATA_START_INDEX and then out the data there
    buffer = (LCD_DATA_PORT & (~(0X0F << LCD_DATA_START_INDEX))) | ((fbp << LCD_DATA_START_INDEX));
    LCD_DATA_PORT = buffer;
    CLEAR_E();
    ...
    SET_E();
    // make a 4 bit hole in LCD_DATA_PORT at the start of LCD_DATA_START_INDEX and then out the data there
    buffer = (LCD_DATA_PORT & (~(0X0F << LCD_DATA_START_INDEX))) | ((sbp << LCD_DATA_START_INDEX));
    LCD_DATA_PORT = buffer;
    CLEAR_E();
}
